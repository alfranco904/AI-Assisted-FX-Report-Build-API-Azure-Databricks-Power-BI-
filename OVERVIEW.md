...
*(screenshot pending — drop `job_trigger_schedule.png` into `docs/images/` showing the
`fx_rates_pipeline` trigger, now paused after the final validation run)*
...
![FX Rate Movement line chart](docs/images/dashboard_line_chart.png)
![FX Spot Rates table](docs/images/dashboard_spot_rates.png)

*(screenshots pending — drop `dashboard_line_chart.png` and `dashboard_spot_rates.png` into
`docs/images/` once captured from Power BI Desktop)*
...
```
FX_Rates_Lab/
├── notebooks/     01_bronze_ingest.py, 02_silver_transform.py, 03_gold_star_schema.py
├── config/        cluster_policy.json, job_definitions.md (Job orchestration spec)
└── docs/          setup runbook, Power BI connection guide, shutdown checklist
```

See `README.md` for the step-by-step build order if reproducing this from scratch.

## Status

Pipeline built and tested end-to-end (Bronze → Silver → Gold), Job scheduled and running, Power BI
dashboard connected and live. Final validation run — a full continuous multi-hour session under
the corrected rate-limit-safe configuration — completed successfully.
